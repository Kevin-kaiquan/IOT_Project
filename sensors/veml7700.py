...
class VEML7700:
    ...
    REG_ALS_CONF = 0x00
    REG_ALS = 0x04
    K = 0.0672  # Calibration constant for VEML7700
    ...
    def _init_sensor(self):
        ...
        try:
            with SMBus(self.busno) as bus:
                conf_val = 0x0000  # Default configuration
                bus.write_i2c_block_data(self.addr, self.REG_ALS_CONF, [conf_val & 0xFF, (conf_val >> 8) & 0xFF])
                time.sleep(0.01)  # Allow configuration to take effect
    ...
    def read_light(self) -> float:
        ...
        try:
            self._init_sensor()
            time.sleep(0.15)  # Wait for integration cycle (sensor needs time to take measurement)
            raw = self._read_raw()
